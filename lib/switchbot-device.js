...
class SwitchbotDevice {
  ...
  * - onlog      | Function | Optional | Function to output logging info
  * ---------------------------------------------------------------- */
  constructor(peripheral, noble, onlog) {
    ...
    let ad = switchbotAdvertising.parse(peripheral, this._onlog);
    this._id = ad.id;
  ...
  _connect() {
    return new Promise((resolve, reject) => {
      ...
      this._peripheral.once('connect', () => {
        if (this._onlog && typeof this._onlog === 'function') {
          this._onlog(`[_connect] device connected`);
        }
      ...
      this._peripheral.once('disconnect', () => {
        if (this._onlog && typeof this._onlog === 'function') {
          this._onlog(`[_connect] device disconnected`);
        }
        this._connected = false;
        this._chars = null;
      ...
      (async () => {
        ...
        for (let service of service_list) {
          ...
          if (typeof this._onlog === 'function') {
            this._onlog(`[_getCharacteristics] discovered characteristics: ${JSON.stringify(char_list.map(char => char.uuid))}`);
          }
    ...
    return new Promise((resolve, reject) => {
      this._peripheral.discoverServices([], (error, service_list) => {
        ...
        if (service) {
          ...
          if (typeof this._onlog === 'function') {
            this._onlog(`[_discoverServices] service uuid "${this._SERV_UUID_PRIMARY}" not found in list ${JSON.stringify(service_list)}!`);
          }
